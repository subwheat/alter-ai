...
def load_model():
    ...
    # Determine which device the model ended up on
    try:
...
def build_chat_input(system_prompt: str, messages: list[ChatMessage], prompt: str) -> str:
    ...
    """
    parts = []
    # System prompt as first user turn (Gemma 4 does not have a dedicated system role)
    if system_prompt:
...
def compute_token_entropy(logits_row: torch.Tensor) -> tuple[float, float, float, float, float]:
    """
    Given a 1D logits vector for one token position, return:
    (logprob, entropy, top1_prob, top2_prob, margin_top1_top2)
    """
    ...
    # logprob of the actual generated token — we'll handle this per-call
...
def generate_one(
    ...
    gen_kwargs = {
        ...
        "output_scores": True,  # get per-step logit distributions
    ...
    # Build per-token trace from scores
    trace = []
    scores = output.scores  # tuple of (1, vocab_size) tensors, one per generated token
    for t_idx, (token_id, score_tensor) in enumerate(zip(generated_ids, scores)):
        logits_row = score_tensor[0]  # shape (vocab_size,)
        probs = torch.softmax(logits_row.float(), dim=-1)
        log_probs = torch.log(probs + 1e-12)
...
async def generate(req: GenerateRequest):
    ...
    # Rough prefill/decode split: first 20% is prefill
    ...
    # Aggregate token metrics per replicate
    token_metrics_per_replicate = [
        aggregate_token_metrics(trace) for trace in per_replicate_traces
    ]

    # Write full token traces to block storage (all replicates concatenated)
    ...
    return GenerateResponse(
        ...
        decode_ms=decode_ms,
        first_token_ms=None,  # requires streaming hooks
